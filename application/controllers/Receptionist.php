...
class Receptionist extends MY_Controller {
	public function __construct() {
		...
		if(!$this->session->userdata('id') && !$this->session->userdata('selection') == "receptionist") {
			redirect('home');
		}
		...
		if($result == 1) {
			$this->power_model->modelLogout($this->session->id, 'receptionist');
	...
	public function appointment() {
		...
		*/
		$permission = 0;
		if($resultAccess = $this->power_model->setAccessPrivilege($this->session->id, 3)) {
	...
	public function appointment_requests() {
		...
		*/
        ...
        $config["base_url"] = base_url() . "receptionist/appointment_requests";
	...
	public function cancelAppointmentRequest() {
		...
		*/
		...
		if(is_numeric($this->uri->segment(3)) == 1) {
			...
			redirect("receptionist/appointment_requests?success");
		} else {
			redirect("receptionist/appointment_requests");
		}
	}
		...
		*/
		...
		if(is_numeric($this->uri->segment(3)) == 1) {
			$datas = array(
				...
				"appointment_approve_selection" => 3
			);
	...
	public function index() {
		...
		foreach($this->power_model->getId($this->session->id, 'receptionist') as $userInfo) {
    		$ReceptionistCode = $userInfo->receptionist_sms_code;
		}
		
		if(strlen($ReceptionistCode) == 6 || strlen($ReceptionistCode) == 5) {
		    redirect('receptionist/verification');
		} 
    ...
    public function verification() {
        $ReceptionistCode = "";
		foreach($this->power_model->getId($this->session->userdata('id'), 'receptionist') as $userInfo) {
    		$ReceptionistCode = $userInfo->receptionist_sms_code;
		}
		
		if(strlen($ReceptionistCode) == 6 || strlen($ReceptionistCode) == 5) {
		} else if($ReceptionistCode == "@@@@@@@@SUCCESS@@@@@@@@") {
        ...
        if($this->form_validation->run() == FALSE) {
		    ...
		    $result = $this->power_model->submitOTP($this->session->id, 'receptionist', $this->input->post('otp'));
	...
	public function messages($code = "") {
		...
		if($code == "") {
			...
			if($this->power_model->checkChatCode($this->session->userdata('id'), $selection, $code) == 0) {
				...
				redirect('receptionist/messages?error=noexists');
			}
		}
	}
	...
	public function delete() {
		...
		*/
		...
		redirect("../receptionist/inquiries_list");
	}
	...
	public function change_profile_picture() {
		...
		if (!$this->upload->do_upload('image')) {
			...
			$this->db->where('receptionist_id', $this->session->id);
       		$this->db->update('receptionists_tbl', $arrayImage);
	...
	public function consultation_requests() {
		...
		*/
		...
		$config = array();
		$config['reuse_query_string'] = true;
        $config["base_url"] = base_url() . "receptionist/consultation_requests";
		...
		*/
		...
		if(is_numeric($this->uri->segment(3)) == 1) {
			$datas = array(
				...
				"consultation_approve_selection" => 3
			);
			$this->power_model->doneConsultation($datas, $this->uri->segment(3));
			redirect("receptionist/consultation?success");
		} else {
	...
	public function transaction_medical_certificate() {
		...
		*/
		...
		$config = array();
		$page = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
        $config["base_url"] = base_url() . "receptionist/transaction_medical_certificate";
	...
	public function transaction_appointment() {
		...
		*/
		...
		$config = array();
		$page = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
        $config["base_url"] = base_url() . "receptionist/transaction_appointment";
	...
	public function transaction_consultation() {
		...
		*/
		...
		$config = array();
		$page = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
        $config["base_url"] = base_url() . "receptionist/transaction_consultation";
	...
	public function immunizationrecord() {
		...
		*/
		...
		$config = array();
        $config["base_url"] = base_url() . "receptionist/immunizationrecord";
	...
	public function addImmunizationRecord($id = "") {
		...
		*/
		if(strlen($id) == 0) {
			redirect("receptionist/immunizationrecord");
		}
		...
		if($this->form_validation->run() == FALSE) {
			...
			redirect("receptionist/immunizationrecord?success");
		}
	}
	...
	public function parents() {
		...
		*/
		...
		$config = array();
        $config["base_url"] = base_url() . "receptionist/parents";
	...
	public function addParents() {
		...
		*/
		...
		if($this->form_validation->run() == FALSE) {
			...
			redirect("receptionist/parents?success");
		}
	}
	...
	public function addChild($id = "") {
		...
		*/
		...
		if($this->form_validation->run() == FALSE) {
			...
			redirect("receptionist/listChild/".$id."/?success");
		}
	}
	...
	public function editChild($id = "") {
		...
		*/
		...
		if($this->form_validation->run() == FALSE) {
			...
			redirect("receptionist/listChild/".$result2[0]['parent_id']."?success");
		}
	}
	...
	public function patients() {
		...
		*/
		...
		$config = array();
        $config["base_url"] = base_url() . "receptionist/patients";
	...
	public function announcements() {
		...
		*/
		...
		$config = array();
        $config["base_url"] = base_url() . "receptionist/announcements";
	...
	public function addAnnouncements() {
		...
		*/
		...
		if($this->form_validation->run() == FALSE) {
			...
			redirect("receptionist/announcements?success");
		}
	}
	...
	public function deleteAnnouncements($id = "") {
		...
		*/
		...
		if(strlen($id) == 0) {
			redirect("receptionist/announcements");
		}
	...
	public function editAnnouncements($id = "") {
		...
		*/
		...
		if($this->power_model->checkAnnouncements($id) == 1) {
			...
			// not exist
			// error
			redirect("receptionist/announcements?error");
		}
	...
	public function viewPediatricChart($patient_id) {
		...
		*/
		if(strlen($patient_id) == 0) {
			redirect("receptionist/viewPediatricChart");
		}
	...
	public function addPediatricChart($id = "") {	
		...
		*/
		...
		if($this->form_validation->run() == FALSE) {
			...
			redirect("receptionist/viewPediatricChart/".$id."?success");
		}
	}
	...
	public function getTransactionAppointmentPDF($per_page, $page) {
		if($per_page == "" && $page == "") {
			redirect("receptionist/index?error=There\'s something wrong!");
		}
		// Get output html
		$html = $this->output->get_output();
	...
	public function medical_certificate() {
		...
		*/
		...
		$config = array();
        $config["base_url"] = base_url() . "receptionist/medical_certificate";
	...
	public function cancelMedicalCertificate() {
		...
		*/
		...
		if(is_numeric($this->uri->segment(3)) == 1) {
			...
			redirect("receptionist/medical_certificate?success");
		} else {
			redirect("receptionist/medical_certificate");
		}
	}
	...
	public function laboratory_results() {
		...
		*/
		...
		$config = array();
        $config["base_url"] = base_url() . "receptionist/laboratory_results";
	...
	public function tabular_reports() {
		...
		*/
		...
		$config = array();
        $config["base_url"] = base_url() . "receptionist/tabular_reports";
